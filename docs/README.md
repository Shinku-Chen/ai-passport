...
| Tuner | Live microphone pitch detection with nearest-note, frequency, and cents readout; debug mode (long-press OK) shows raw frequency / RMS / NSDF and adjusts mic gain | `main/demo_tuner.c`, `main/tuner_engine.c`, `components/bsp/src/bsp_audio.c` | Reuses the microphone over `bsp_audio_*`; accuracy and responsiveness require on-device verification; pitch detection is pure host-testable C |
