...
- AI Passport 上《ATRI -My Dear Moments-》竖屏阅读器首发：34 章、1,069 幕、12,188 句对白、5 张全身立绘、三个结局，完全离线。3.8 MB 资源包与 16px 中文字体子集都编进固件，烧录即可玩。
- 剧本与素材取自重构版手环同人移植 `fywmjj/better-mb9p-ATRI`（补上了旧移植缺少的 `src/common/character/*` 全身立绘）；剧本与背景与 `liuyuze61/ATRI-miband` 一致。
- 竖屏 240x320 版面：整屏背景 JPEG + 源工程同款半透明蓝底文本带（在像素层混进画布）+ 说话人名牌 + 左上章节号 + 右下页码。立绘合成在文本带之上，文字区只盖一层浅暗帘，人物不会再被对话框挡住。
- 立绘节奏：只有"这句有说话人名字"时才显示对应立绘，说完即收起；事件 CG、黑屏与男主（夏生，视角角色）都不显示立绘。剧本里同一人的多种写法（机器人少女 / 少女 / 某人的声音 / 凛凛花的三种写法 / 竜司 / キャサリン）都映射到同一张立绘。
- 三键操作：**上 / 下短按**下一句，**长按上 / 下**快进（整句直显、松手即停），**确定**打开菜单（保存 / 读取 / 跳过章节 / 返回标题）。列表页上 / 下选择 + 确定，长按确定返回。文字速度可选 瞬间 / 慢 / 中 / 快，关于页用上 / 下滚动。
- 存档：5 个手动槽 + 每次换幕自动写入的自动槽（标题页"继续阅读"读它）；新的 v2 存档格式会连当前立绘一起保存，并仍能读旧版 v1 存档；圆满 + 悲剧结局都达成后，标题页解锁"真正的结局"章节。
- 空闲行为：45 秒调暗、150 秒熄屏、7 分钟深睡，任意键唤醒。进入深睡时会把 ES8311 codec 一并置入低功耗，消除功放上能听到的蜂鸣声。
- 便于后续迭代的验收手段：串口命令 `ATRIJUMP <章下标> [幕下标]`（直接拨进度）、`ATRISHOT <章下标> <幕下标>`（把该幕渲染进画布并回传 240x320 原始帧），以及 `idf.py -DATRI_BOOT_CHAPTER=<下标> build` 开机直接进某一章。
