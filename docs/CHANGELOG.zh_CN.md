...
- 调音器性能修复（真机验证中发现）：NSDF 自相关热循环使用 `float` 运算，在无 FPU 的 ESP32-C3 上走软浮点库，单窗检测耗时秒级，触发任务看门狗（task_wdt）超时并卡住 UI。已将热循环与 RMS 门限改为 `int64` 整数运算（算法与阈值不变）；单窗检测降至毫秒级。
- 调音器高音修复（真机验证中发现）：`TUNER_MAX_FREQ` 原为 1200 Hz，手机/钢琴音源中高于它的音（如 E6=1318 Hz）落在检测范围外，NSDF 扫描从真实周期之下的 `lag_min` 起步，选到 2× 周期 → 高音被低八度误判。已将检测上限提高到 2000 Hz，并补充 C6/E6/G6/A6 host 测试用例。
