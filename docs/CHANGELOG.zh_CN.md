...
- 新应用：**四子棋**开机直接进入横屏（320 × 240）对局，棋盘 10 列 × 7 行共 70 格。三种模式——玩家先手、电脑先手（两种都带低 / 中 / 高三档难度）与双人同机轮下；落子预览可选“真实落点”或“只在该列最上一行”；另有设置屏切换模式 / 难度 / 预览、代码合成的落子/胜/负/平/换列音效，以及空闲自动深睡（设置屏 60 秒、对局中 180 秒，任意键唤醒）。
