...
3. **验证完整固件**：脚本打包 `voicefs` 数据分区（来自已提交片段），逐字节确认 bootloader、partition-table、app 和 voicefs 数据分别位于 `0x0`、`0x8000`、`0x10000` 和 `0x210000`，确认 `flash_args` 使用 8 MB Flash，并输出可从 `0x0` 直刷的 `build/FoloToy-AI-Passport-full.bin`。
