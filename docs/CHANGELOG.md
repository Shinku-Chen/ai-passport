...
- Disabled the Bluetooth/NimBLE stack (`CONFIG_BT_ENABLED=n`) since the sound-keychain product app does not use BLE, saving flash/RAM and removing a potential runtime power source (deep sleep powers the controller down anyway; this trims the running build). Also disabled `CONFIG_ESP_SLEEP_GPIO_ENABLE_INTERNAL_RESISTORS` because GPIO0 relies on the external 10 kΩ pull-up, so the stacked internal pull-up was pure leakage. App image shrank ~4 KB.
...
- Fixed list-page playback: pressing OK stopped the current sound but the selected one did not play, because each play ran `xTaskCreate` for a 16 KB Opus-decode stack, which failed intermittently under heap pressure/fragmentation. Replaced it with a single persistent player task that uses a **static stack** and is woken by a binary semaphore, so OK always stops the current sound and starts the selected one. Verified on device.
- Fixed the battery percentage display. The top bar SOC was read once at boot and never refreshed, going stale as the battery drained or charged — it now re-reads the gauge every 30 s. Also, on some power cycles the CW2017 returns `0xFF` in its SOC register (no SOC computed), which showed as 0%; `bsp_battery_soc()` now falls back to a single-cell Li-Poly open-circuit-voltage → SOC estimate in that case (a 4073 mV battery now reports 94% instead of 0%). Verified on device.
- Fixed the deep-sleep wakeup so the sound keychain actually wakes on a key press. `esp_deep_sleep_enable_gpio_wakeup()` takes a **pin bitmask**, but the code passed the raw pin number `GPIO_NUM_0` (value `0`); the function treats a zero mask as invalid and returns without arming any wake source, so the chip slept forever and GPIO0 button presses could never wake it. It now passes `1ULL << GPIO_NUM_0` and checks the return value (logs an error instead of sleeping silently). Verified on-device: sleeps after 5 min idle and wakes on any button.
...
- Made mini-program BLE install compatibility a template-level invariant: fixed protected `cardid`/Recovery partitions, retained the five-second UP-key Recovery boot hook, and added CI validation for merged-image structure, partition MD5/ranges, the 3 MB app limit, and protected payload exclusion.
