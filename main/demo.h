...
typedef struct {
    ...
    void (*key)(bsp_btn_t btn, bsp_btn_ev_t ev);  // 收按键(长按确定默认被 main 拦截)
    bool consumes_long_ok;                        // true=长按 OK 不进"返回菜单",直接交给 key()
} demo_entry_t;
...
void demo_tuner_enter(void);     void demo_tuner_exit(void);
