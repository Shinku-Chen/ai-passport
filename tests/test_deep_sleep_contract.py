...
class DeepSleepContractTest(unittest.TestCase):
    ...
    def test_terminal_shutdown_order_precedes_deep_sleep(self) -> None:
        body = function_body(self.app, "do_deep_sleep")
        calls = [
            "bsp_battery_sleep()",
            "bsp_audio_sleep()",
        ...
        # 应用侧必须先在共享总线设备做完事务后再释放引脚, 并停止播放/使用终端接口。
        self.assertIn("stop_playback()", body)
        self.assertNotIn("bsp_display_sleep()", body)
