...
把小米手环 9 上的《ATRI -My Dear Moments-》同人移植
（[`liuyuze61/ATRI-miband`](https://github.com/liuyuze61/ATRI-miband)）搬到了 AI Passport 上的
**竖屏视觉小说阅读器**：**34 章、1,069 幕、12,188 句对白、三个结局**，完全离线。原版是 Vela OS
的触屏应用，本分支用 C + LVGL 重写了阅读引擎，改成三键操作。
...
│ │ 亚托莉 │                       │
│ └────────┴───────────────────────┤  源工程的半透明蓝底文本带
│  正文,5 行 × 13 个全角字/行      │  直接画进画布,立绘能透出来
│                                  │  16px 字体,20px 行距
└──────────────────────────────────┘
```
...
| 步骤 | 工具 | 产物 |
| --- | --- | --- |
| 剧本 + 图像 | `tools/atri_pack.py` | `main/atri_data/atri_pack.bin`（3.59 MB）：34 章、1,069 幕、12,188 句、73 张整屏背景、16 张叠加，外加来源元数据 |
...
- **不占 RAM 的叠加层** —— 16 张人物 / 效果叠加按 alpha 包围盒裁剪，无损存成
...
- **文本带画进画布** —— 源工程用一张半透明的蓝色 `text_bg` 盖在整屏画面上；这里把同一条带
  子（同样的颜色、同样的自上而下渐深）在像素层混进画布，所以铺到屏幕底部的立绘会透出来，
  而不是被一个不透明文本框硬切掉。
